...
dlgRubroProveedorEdit::dlgRubroProveedorEdit(int idProveedor, RubrosProveedoresModel *model, int selectedEntity, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::dlgRubroProveedorEdit)
{
    ui->setupUi(this);
    _model = model;
    ui->cboRubro->setModel(GlobalContainer::instance().library()->model(Tables::Rubros));
...
dlgRubroProveedorEdit::~dlgRubroProveedorEdit()
{
    delete ui;
}

void dlgRubroProveedorEdit::on_buttonBox_accepted()
{
    RubroProveedorPtr rubroProveedor = qSharedPointerDynamicCast<RubroProveedor>(_entity);
